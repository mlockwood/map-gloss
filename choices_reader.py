...
### Updated to python3.4: ChoicesReader.py for AGGREGATION and LING575 (SP2015) 2015-05-21 Olga Zamaraeva olzama@uw.edu
...
# EMB 2015-05-21: These are what was being imported (& used) from
# elsewhere in gmcs.  Trying just putting it directly here rather than
# bringing over more modules.
...
class ChoiceCategory:
  ...
  def get(self, key, default=None):
    ...
    #except TypeError:
        #TODO: not sure what to do here, just copying from above IndexError handling
        #x = default if default is not None else ''
    # reset safe_get
    self.safe_get = True
    return x
...
class ChoiceList(ChoiceCategory, list):
  ...
  def __getitem__(self, key):
    ...
    #
    # olzama 2015-05-26: I see this error when we try to retrieve a None item.
    # Due to all the overwritten methods, this is hard to debug and assess,
    # and my impression is that some places in the code do not expect the None items
    # and don't handle them.
  ...
  '''
  olzama: Boy, does this screw things up...
  Very hard to debug with None items being skipped and not reflected in len() etc.
  I also suspect that there are bugs due to None items not expected/handled properly.
  '''
  def __iter__(self):
    """
...
class ChoicesFile:
  ...
  def features(self):
    ...
    # Case
    features += self.__get_features(case_names(self), 0, 1, 'case',
  ...
  def convert_key(self, old, new, key_prefix=''):
    ...
    if old in self:
    #    if not new in self or isinstance(self[new],str):
        self[new] = self[old]
     #   else:
     #       self[new].append(self[old])
